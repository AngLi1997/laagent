<template>
  <div class="flow-tool-node">
    ...
    <div v-if="showName && showName.length" class="bottom">
      {{ showName }}{{ showMCPName.length ? `-${showMCPName}` : '' }}
    </div>
  </div>
</template>
...
import { reqAgentAgentsToolList } from '@/api';
...
const showName = computed(() => {
  return toolList.value.find(item => item.id === curFormData.value[`${FlowNodeEnum.TOOL}_config`]?.id)?.name || '';
});

const showMCPName = computed(() => {
  return curFormData.value[`${FlowNodeEnum.TOOL}_config`]?.tool_name || '';
});
...
onMounted(() => {
  ...
  node.value.on('change:data', ({ current }: any) => {
    ...
    if (formData[`${FlowNodeEnum.TOOL}_config`] && formData[`${FlowNodeEnum.TOOL}_config`]?.id) {
