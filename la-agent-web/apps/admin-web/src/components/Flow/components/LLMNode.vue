<template>
  <div class="flow-LLM-node">
    ...
    <div v-if="curFormData.llm_config && curFormData.llm_config.name" class="bottom">
      {{ curFormData.llm_config.name }}
    </div>
  </div>
</template>
...
onMounted(() => {
  ...
  node.value.on('change:data', ({ current }: any) => {
    ...
    if (formData.llm_config && formData.llm_config.name) {
